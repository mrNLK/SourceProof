...
-- Create settings key-value table (idempotent)
...
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'settings' AND policyname = 'Allow public insert settings') THEN
    CREATE POLICY "Allow public insert settings" ON public.settings FOR INSERT WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'settings' AND policyname = 'Allow public update settings') THEN
